...
def deleteExistingApplications():
	...
	deleteRequest = "cf delete -f -r " +dataSeedAppName
	statementStatus  = subprocess.call(deleteRequest, shell=True)
	if statementStatus == 1 :
		print("Error creating a deleting service instance" +deleteRequest)
		

	deleteRequest = "cf delete -f -r " +dataSourceAppName
	statementStatus  = subprocess.call(deleteRequest, shell=True)
	if statementStatus == 1 :
		print("Error creating a deleting service instance" +deleteRequest)
		
	deleteRequest = "cf delete -f -r " +httpDataRiverAppName
	statementStatus  = subprocess.call(deleteRequest, shell=True)
	if statementStatus == 1 :
		print("Error creating a deleting service instance" +deleteRequest)	
		
	deleteRequest = "cf delete -f -r " +dataIngestionAppName
	statementStatus  = subprocess.call(deleteRequest, shell=True)
	if statementStatus == 1 :
		print("Error creating a deleting service instance" +deleteRequest)		
		
	deleteRequest = "cf delete -f -r " +machineSimulatorAppName
	statementStatus  = subprocess.call(deleteRequest, shell=True)
	if statementStatus == 1 :
		print("Error creating a deleting service instance" +deleteRequest)
		
	deleteRequest = "cf delete -f -r " +uiAppName
	statementStatus  = subprocess.call(deleteRequest, shell=True)
	if statementStatus == 1 :
		print("Error creating a deleting service instance" +deleteRequest)	
...
def deployAndBindUAAToPredixBoot():
	...
	buildProject('mvn clean install -DskipTests=true',projectDir)
...
def checkoutAndDeploy(repoName,manifestLocation,appDeploymentName):
	...
	buildProject('mvn clean install',repoName)
	configureManifest()
...
checkoutAndDeploy(dataSeedRepoName,"dataseed-service",dataSeedAppName)

dataSourceRepoName = "rmd-datasource"
checkoutAndDeploy(dataSourceRepoName,"datasource-service",dataSourceAppName)


cfTarget= subprocess.check_output(["cf", "app",dataSourceAppName])
print (cfTarget)
...
postgreJsonrequest = "cf cs "+predixPostgre+" "+predixPostgrePlan+" "+rmdPostgre
...
checkoutAndDeploy(websocketServerRepoName,"",websocketAppName)

cfTarget= subprocess.check_output(["cf", "app",websocketAppName])
print (cfTarget)
...
checkoutAndDeploy(dataIngestionRepoName,"",dataIngestionAppName)


cfTarget= subprocess.check_output(["cf", "app",dataIngestionAppName])
print (cfTarget)
...
checkoutAndDeploy(machineSimulatorRepoName,"",machineSimulatorAppName)
...
checkoutAndDeployUI(uiRepoName,"",uiAppName)

print("********* DONE deploying UI application ********************")



cfTarget= subprocess.check_output(["cf", "app",dataSeedAppName])
print (cfTarget)
dataSeedUrl="https://"+cfTarget.split('urls:')[1].strip().split('last uploaded:')[0].strip()
print ('Data Seed URL '+dataSeedUrl)


jsonrequest = "cf apps | grep "+instanceAppender
statementStatus  = subprocess.call(postgreJsonrequest, shell=True)

jsonrequest = "cf s | grep "+ instanceAppender
statementStatus  = subprocess.call(jsonrequest, shell=True)

time.sleep(30)
stopSimulatorRequest = "cf stop "+machineSimulatorAppName
statementStatus  = subprocess.call(stopSimulatorRequest, shell=True)


print ('Visit the DataSeedService to load data in to the Predix Reference App.  e.g.'+dataSeedUrl)
